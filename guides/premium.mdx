---
title: Premium
sidebarTitle: Premium
description: Learn how to buy Premium and what features it has
icon: crown
iconType: duotone
---

<script dangerouslySetInnerHTML={{
  __html: `
    (function() {
      let isToggling = false;
      
      document.addEventListener('click', function(e) {
        const summary = e.target.closest('summary');
        if (summary && !isToggling) {
          const clickedAccordion = summary.parentElement;
          const wasOpen = clickedAccordion.hasAttribute('open');
          
          isToggling = true;
          
          if (!wasOpen) {
            const allAccordions = document.querySelectorAll('details[open]');
            allAccordions.forEach(accordion => {
              if (accordion !== clickedAccordion) {
                accordion.removeAttribute('open');
              }
            });
          }
          
          requestAnimationFrame(() => {
            isToggling = false;
          });
        }
      });
    })();
  `
}} />

<div className="mobile-only">
<Card title="Buy Premium" icon="circle-1" href="../guides/premium#how-to-buy-premium-on-haunt-gg" horizontal>
Learn how to buy premium on haunt.gg
</Card>

<Card title="Payment Methods" icon="circle-2" href="../overview/products#payment-methods" horizontal>
Check out the available payment methods.
</Card>

<Card title="Premium Features" icon="circle-3" href="../guides/premium#premium-features" horizontal>
Check out all premium features.
</Card>
</div>

## How to buy Premium on haunt.gg

<Info>
Available payment methods can be found [here](../overview/products#payment-methods).
</Info>

<Steps>
    <Step title="Go to the pricing page">
        Head over to [haunt.gg/pricing](https://haunt.gg/pricing#premium)
    </Step>
    <Step title="Pick your payment method">
        Then pick your preferred payment method
    </Step>
</Steps>

---

## Premium Features

<AccordionGroup>

<Accordion title="Claim Usernames">
Premium allows you to **claim** banned or inactive usernames. To claim a username, go into the `#ticket`
channel in our [Discord server](https://discord.gg/hauntgg) and select **<Icon icon="clipboard-user" color="f27db3" size="1em"/> User Claim**.
You will be prompted with a modal to fill out the name you want to claim.

Some usernames can be automatically claimed without manual review. If so, the bot will tell you.

<Frame caption="Example of direct username claim">
    ![Claim Usernames](../assets/images/username-claim-dc.png)
</Frame>
</Accordion>

<Accordion title="Alias">
You receive an alias token after buying premium, [**learn more**](../overview/products#alias-token).
</Accordion>

<Accordion title="Custom Domain">
Use your own domain to display your haunt.gg profile.

<Frame>
    ![Domain Settings](../assets/images/premium-domain-settings.png)
</Frame>

<Info>
    Adding a custom domain works like [__here__](../guides/imagehost#domain-settings-custom-domain).
</Info>
</Accordion>

<Accordion title="Profile Metadata">
You can set your **Discord Embed**, **Tab Title**, and **Favicon** with Profile Metadata.  

<Frame>
    ![Profile Metadata](../assets/images/profile-metadata-premium.png)
</Frame>

<Frame>
    ![Metadata Visibility](../assets/images/profile-metadata-visibility.png)
</Frame>

### Metadata Preview (Discord)

<Frame>
    ![Profile Metadata Discord](../assets/images/profile-metadata-discord-premium.png)
</Frame>
</Accordion>

<Accordion title="Typewriter Descriptions">
You can add multiple descriptions to display on your Bio with a **Typewriter effect**.  

<Frame>
    ![Typewriter Descriptions](../assets/images/typewriter-descriptions-premium.png)
</Frame>

### Typewriter Effect Preview

<Frame>
    ![Typewriter Descriptions Website](../assets/images/typewriter-description-premium-website.gif)
</Frame>
</Accordion>

<Accordion title="Cursor Effects">
You can add a **custom cursor effect** that follows your mouse on your Bio.  

<Frame>
    ![Cursor Effects](../assets/images/cursor-effects-premium.png)
</Frame>

### Cursor Effect Preview

<Frame>
    ![Cursor Effects Website](../assets/images/cursor-effects-premium-website.gif)
</Frame>
</Accordion>

<Accordion title="Font Settings">
You can change the **font** for your Bio page (including your name and description).  

<Frame>
    ![Font Settings](../assets/images/font-settings-premium.png)
</Frame>

### Font Preview

<Frame>
    ![Font Settings Website](../assets/images/font-settings-premium-website.png)
</Frame>
</Accordion>

<Accordion title="Profile Animation">
You can add a **custom animation** to your Bio page.  

<Frame>
    ![Profile Animation](../assets/images/profile-animation-premium.png)
</Frame>

### Profile Animation Preview

<Frame>
    ![Profile Animation Website](../assets/images/profile-animation-premium-website.gif)
</Frame>
</Accordion>

<Accordion title="Profile Enter Animation">
You can add a **custom enter animation** to your Bio page.  

<Frame>
    ![Profile Enter Animation](../assets/images/profile-enter-animation.png)
</Frame>

### Profile Enter Animation Preview

<Frame>
    ![Profile Enter Animation Preview](../assets/images/profile-enter-animation-example.gif)
</Frame>
</Accordion>

<Accordion title="Profile Sound Effect">
You can add a **click sound effect** to your Bio page. You can also addd a custom sound. 

<Frame>
    ![Profile Sound Effect](../assets/images/profile-sound-effect.png)
</Frame>
</Accordion>

<Accordion title="Enter Screen Customization">
Add further enter screen customization to your profile.

<Frame>
    ![Enter Screen Customization](../assets/images/enter-screen-premium.png)
</Frame>
</Accordion>

<Accordion title="Username Cooldown">
Premium removes the **username cooldown**, letting you change your username anytime.  

<Frame>
    ![Username Cooldown](../assets/images/username.png)
</Frame>
</Accordion>

<Accordion title="Customizable Border Options">
You can customize your border’s **animations**.  

<Frame>
    ![Customizable Border Options](../assets/images/border_animations.png)
</Frame>

### Animated Border Preview

<Frame>
    ![Customizable Border Options Website](../assets/images/border_animation.gif)
</Frame>
</Accordion>

<Accordion title="Custom Fonts">
You can upload and use **custom fonts** on your Bio page (including your name and description).  

<Frame>
    ![Custom Fonts](../assets/images/custom_fonts.png)
</Frame>
</Accordion>
</AccordionGroup>

---

## How do I get the premium rank on Discord?

<Note>
     You can learn more about how to link your rank to Discord in this guide: [Discord Connection](/guides/discord)
</Note>

